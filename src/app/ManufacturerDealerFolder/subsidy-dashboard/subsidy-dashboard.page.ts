...
@Component({
  selector: 'app-subsidy-dashboard',
  templateUrl: './subsidy-dashboard.page.html',
  styleUrls: ['./subsidy-dashboard.page.scss'],
})
export class SubsidyDashboardPage {
  val: any = '1';
  ...
  constructor(public platform: Platform, public dbService: DatabaseServiceService, public navCtrl: NavController, private storage: Storage, public router: Router, public route: ActivatedRoute,
  ...
  ionViewWillEnter() {
    ...
    this.storage.get('userData').then(res => {
      ...
      this.roleName_EN = res.RoleName_EN;
      console.log("this.roleId----1", this.roleId);
      console.log("this.userid-----1", this.userid);
      console.log("this.roleName_EN-----1", this.roleName_EN);
    ...
    //
    // });
    // this.storage.get('roleName_EN').then(res => {
    //   this.roleName_EN = res;
    //   console.log("this.roleName_EN---2", this.roleName_EN);
    //
    // });
  ...
  getSubsidyDashboard(roleId, userid) {
    ...
    this.httpClient.post(sendRequestData).subscribe(function (res: any) {
      ...
      if (res[0].status == 0) {
        ...
        console.log("self.getHortiSubsidyDashboard", self.getHortiSubsidyDashboard);
        self.httpClient.dismissLoadingImage();
        if (self.header == '1') {
          for (var i = 0; i < self.getHortiSubsidyDashboard.length; i++) {
            self.totalPending += parseInt(self.getHortiSubsidyDashboard[i].TotalPending);
            self.totalComplete += parseInt(self.getHortiSubsidyDashboard[i].TotalCompleted);
            self.total = self.totalPending + self.totalComplete;
          }

        } else {
          for (var j = 0; j < self.getHortiSubsidyDashboard.length; j++) {
            self.totalPending += parseInt(self.getHortiSubsidyDashboard[j].TotalDealerAssignPending);
            self.totalComplete += parseInt(self.getHortiSubsidyDashboard[j].TotalDealerAssignCompleted);
            self.total = self.totalPending + self.totalComplete;
          }
        }
      } else {
  ...
  saveDashBoardOffline(data) {
    for (var i = 0; i < data.length; i++) {
      this.dbService.addSubsidyScheme(data[i].SubsidySchemeId, data[i].SubsidySchemeHi, data[i].SubsidySchemeEn, data[i].DashboardImg);
    }
  }
  onSubsidyClick(subsidyId) {
    console.log("subsidyId", subsidyId);

    this.router.navigate(['horticulture-application-list', { subsidyId: subsidyId }]);

  }


  assignedToDealer(clickOpt) {
    console.log("subsidyId", clickOpt);

    this.router.navigate(['assign-to-dealer-application-list', { clickOpt: clickOpt }]);
  ...
  async showPrompt(msg) {
    if (this.httpClient.currentLanguage == 'hindi') {
      ...
      const prompt = await this.alertCtrl.create({
        ...
        buttons: [{
          text: 'रद्द करें', role: 'cancel', handler: () => {
          }
        },
        {
          text: 'ठीक है',
