...
@Component({
    selector: 'app-drip-post-verification',
    templateUrl: './drip-post-verification.page.html',
    styleUrls: ['./drip-post-verification.page.scss'],
})
export class DripPostVerificationPage {
    remarks: any = '';
    ...
    takePhotoNoCase(param, columnName) {
        ...
        this.camera.getPicture(this.httpClient.options).then(
            (imageData) => {
                ...
                if (this.httpClient.isOffline) {
                    this.dbService.storage.executeSql("SELECT * FROM postVerificationOfflineOfDrip WHERE ApplicationId = ?", [this.data.ApplicationId]).then((res) => {
                        console.log("second");
                        var temp = JSON.parse(res.rows.item(0).applicationData);
                        console.log("third");
                        if (param == 1) {
                        ...
                        let data = temp;
                        this.dbService.storage.executeSql(`UPDATE postVerificationOfflineOfDrip
