...
@Component({
    selector: "app-mini-sprinkler-post-verification",
    templateUrl: "./mini-sprinkler-post-verification.page.html",
    styleUrls: ["./mini-sprinkler-post-verification.page.scss"],
})
export class MiniSprinklerPostVerificationPage {
    remarks: any = "";
    ...
    constructor(
        ...
        public camera: Camera,private ngZone: NgZone,
    ...
    getMenuPrototype(ev) {
        console.log('RS getMenuPrototype ev Agri_HortiManufacturerId', ev);
        for (let i = 0; i < this.getHorticultureManufactureData.length; i++) {
            if (this.getHorticultureManufactureData[i].Agri_HortiManufacturerId == ev) {
                var manufacturerId = this.getHorticultureManufactureData[i].Agri_HortiManufacturerId;
                ...
                console.log('RS getMenuPrototype ev2', ev);
    ...
    getAddSprinklerPostVerification() {
        ...
        // } 
            ...
            if (this.httpClient.isOffline) {
                this.dbService.storage.executeSql("SELECT * FROM postVerificationOfflineOfMiniSprinkler WHERE ApplicationId = ?",
                    [this.data.ApplicationId]).then((res) => {
                        ...
                        this.dbService.storage.executeSql(`UPDATE postVerificationOfflineOfMiniSprinkler SET applicationData = ? WHERE ApplicationId = ${this.data.ApplicationId}`, data)
                            .then(() => {
    ...
    takePhotoNoCase(param, columnName) {
        ...
        this.camera.getPicture(this.httpClient.options).then(
            (imageData) => {
                ...
                if (this.httpClient.isOffline) {
                    this.dbService.storage.executeSql("SELECT * FROM postVerificationOfflineOfMiniSprinkler WHERE ApplicationId = ?", [this.data.ApplicationId]).then((res) => {
                        console.log("second");
                        var temp = JSON.parse(res.rows.item(0).applicationData);
                        console.log("third");
                        if (param == 1) {
                        ...
                        let data = temp;
                        this.dbService.storage.executeSql(`UPDATE postVerificationOfflineOfMiniSprinkler
    ...
    checkAvailSubsidy(i){
        ...
        temp=this.ChecklistDetailList.filter((x)=> x.ChecklistId == "60");
        if(temp.length>0){
    ...
    /*
        getCML(i, ComponentName) {
            ...
            var sendRequestData = {
                obj: {
                    ...
                    srvparam: JSON.stringify({
                        ...
                        ISI_Mark: this.getHorticultureCouplerComponentListData[i].ISI_MarkID,
                    }),
                },
            };
            var sendRequestData2 = JSON.stringify({
                obj: {
                    usrnm: "rajkisan",
                    psw: "rajkisan@123",
                    srvnm: "PostVerification",
            ...
            console.log("sendRequestData2", sendRequestData2);
            this.httpClient.post(sendRequestData).subscribe(
                function (res: any) {
                    console.log("res", res);
                    self.httpClient.dismissLoading();
    ...
    */
            ...
            this.httpClient.post(sendRequestData).subscribe(
                function (res: any) {
                    ...
                    if (res[0].status == 0) {
                        ...
                        var optionalItemGroupId1 = self.getPVOptionalItemListData.filter((x) => x.OptionalItemGroupId == '1');
                        var optionalItemGroupId2 = self.getPVOptionalItemListData.filter((x) => x.OptionalItemGroupId == '2');
                        var optionalItemGroupId3 = self.getPVOptionalItemListData.filter((x) => x.OptionalItemGroupId == '3');
    ...
    pickImage(sourceType,param, columnName) {
        ...
        this.camera.getPicture(options).then((imageData) => {// this.httpClient.showLoading();
                ...
                if (this.httpClient.isOffline) {
                    this.dbService.storage.executeSql('SELECT * FROM postVerificationOfflineOfDrip WHERE ApplicationId = ?',
                        [this.data.ApplicationId]).then((res) => {
                        var temp = JSON.parse(res.rows.item(0).applicationData);
                        ...
                        let data = [JSON.stringify(temp)];
                        this.dbService.storage.executeSql(`UPDATE postVerificationOfflineOfDrip SET applicationData = ? WHERE ApplicationId = ${this.data.ApplicationId}`, data)
                            .then(() => {
                                this.ImgTriPartyAgreement = imageData;
                            })
                            .catch((e) => {
