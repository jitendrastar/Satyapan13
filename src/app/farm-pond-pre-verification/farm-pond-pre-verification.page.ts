...
@Component({
    selector: "app-farm-pond-pre-verification",
    templateUrl: "./farm-pond-pre-verification.page.html",
    styleUrls: ["./farm-pond-pre-verification.page.scss"],
})
export class FarmPondPreVerificationPage implements OnInit {
    ...
    constructor(
        ...
        if (this.httpClient.isOffline) {
            ...
            if (this.getPreVerificationData.mainDataSubmit) {
                ...
                if (this.getPreVerificationData.imageOFimgFarmerAndOfficerWithFarmPond) {
                    this.mainDataSubmittedFlag = true;
                    this.imgFarmerAndOfficerWithFarmPond = this.getPreVerificationData.imageOFimgFarmerAndOfficerWithFarmPond;
    ...
    submitMainData() {
        if (this.httpClient.latitude == null) {
            ...
            if (this.httpClient.currentLanguage == "english") {
                this.httpClient.showToast(
                    "Please enter the Suitable khasara for farm pond"
                );
            } else {
                this.httpClient.showToast(
                    "कृपया खेत तालाब के लिए उपयुक्त खसरा दर्ज करें"
                );
            }
        } else if (
            ...
            if (this.httpClient.isOffline) {
                ...
                var query = this.dbService.storage.executeSql(`
    ...
    selectImage(flag, columnName) {
        ...
        this.camera.getPicture(this.httpClient.options).then((imageData) => {
                ...
                if (this.httpClient.isOffline) {
                    this.dbService.storage.executeSql("SELECT * FROM preVerificationOfflineFormPond WHERE ApplicationId = ?", [this.getPreVerificationData.ApplicationId]).then((res) => {
                        ...
                        if (flag == 0) {
                            ...
                            this.dbService.storage.executeSql(`UPDATE preVerificationOfflineFormPond SET applicationSubmissionData = ? WHERE ApplicationId = ${this.getPreVerificationData.ApplicationId}`, data)
                                .then(() => {
                            ...
                            temp.imageOFimgFarmerAndOfficerWithFarmPond = imageData;
                            ...
                            console.log("temp", temp);
                            let data = [JSON.stringify(temp)];
                            this.dbService.storage.executeSql(`UPDATE preVerificationOfflineFormPond SET applicationSubmissionData = ?
        ...
        if (this.httpClient.latitude == null) {
            ...
            if (this.httpClient.isOffline) {
                ...
                this.dbService.storage
                    .executeSql(
                        "SELECT * FROM preVerificationOfflineFormPond WHERE ApplicationId = ?",
                        [this.getPreVerificationData.ApplicationId]
                    ).then((rews) => {
                    ...
                    var errorRequestData = {
                        'obj': {
                            ...
                            srvparam: JSON.stringify({
                                ...
                                ApplicationId: self.getPreVerificationData.ApplicationId,
                                ImgFarmerAndOfficerDateTime: self
                                    .imgFarmerWithOfficerNoCaseDateTime,
                                roleid: self.httpClient.userData.roleid,
                                userid: self.httpClient.userData.userid,
                                schemeid: self.getPreVerificationData.SubsidySchemeId,
    ...
    submitFinal() {
        if (this.farmerChoice == "yes") {
            if (this.najariyaNaksha == null) {
                ...
                if (this.httpClient.currentLanguage == "english") {
                    this.httpClient.showToast(
                        "Please upload photo Farmer and officer with farm pond"
                    );
                } else {
                    this.httpClient.showToast(
                        "कृपया किसान और अधिकारी खेत तलाई के साथ फोटो अपलोड करें "
                    );
                }
            } else {
                var self = this;
